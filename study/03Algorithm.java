 ...
 - A와 B는 friends의 원소이며 A와 B가 같은 이름인 경우는 존재하지 않습니다.
  ...
  class Solution {
    public int solution(String[] friends, String[] gifts) {
        Map<String, Integer> giftsGiven = new HashMap<>();
        Map<String, Integer> giftsReceived = new HashMap<>();
        
        for (String friend : friends) {
            giftsGiven.put(friend, 0);
            giftsReceived.put(friend, 0);
        }
        ...
        for (String gift : gifts) {
            ...
            giftsGiven.put(giver, giftsGiven.get(giver) + 1);
            giftsReceived.put(receiver, giftsReceived.get(receiver) + 1);
        }

        Map<String, Integer> giftIndex = new HashMap<>();
        for (String friend : friends) {
            int given = giftsGiven.get(friend);
            int received = giftsReceived.get(friend);
        ...
        for (String gift : gifts) {
            ...
            int giverToReceiver = giftsGiven.get(giver) - giftsReceived.get(receiver);
            int receiverToGiver = giftsGiven.get(receiver) - giftsReceived.get(giver);

            if (giverToReceiver > receiverToGiver) {
                nextMonthGifts.put(receiver, nextMonthGifts.get(receiver) + 1);
        ...
        for (int i = 0; i < friends.length; i++) {
            for (int j = i + 1; j < friends.length; j++) {
                ...
                if (!giftsGiven.containsKey(friendA + " " + friendB) &&
                    !giftsGiven.containsKey(friendB + " " + friendA)) {
